<template>
  ...
  <b-modal id="show_draft_sales" size="lg" hide-footer :title="$t('Draft_Sales')">
    <div>
  ...
  <b-modal id="modal_today_sales" hide-footer size="lg" :title="$t('Today_Sales')">
      ...
      <div class="ts-card">
        ...
        <div v-if="paginated_Products.length > 0" class="pagination-footer">
          ...
          <div class="pagination-dots">
            <button
              v-for="page in Math.ceil(product_totalRows / product_perPage)"
              :key="page"
              class="pagination-dot"
              :class="{ active: page === product_currentPage }"
              @click="Product_onPageChanged(page)"
              :title="`${$t('pos.Go_to_page')} ${page}`"
            >
              {{ page }}
            </button>
          </div>
          ...
          <button
            class="pagination-btn"
            @click="Product_onPageChanged(product_currentPage + 1)"
            :disabled="product_currentPage === Math.ceil(product_totalRows / product_perPage)"
    ...
    <b-modal id="OpenRegisterModal" :title="$t('Open Register')" hide-footer>
      ...
      <div class="form-group">
        ...
        <input type="number" min="0" step="0.01" class="form-control" v-model.number="registerForm.opening_balance" />
      </div>
      <div class="form-group">
        <label>{{$t('notes')}}</label>
    ...
    <b-modal id="CloseRegisterModal" :title="$t('Close Register')" hide-footer>
      <div class="form-group">
        ...
        <input type="number" min="0" step="0.01" class="form-control" v-model.number="closeForm.counted_cash" />
      </div>
      <div class="form-group">
...
export default {
  ...
  methods: {
    ...
    // ✅ If PayHere payment, show pending message instead of invoice
    if (evt?.requires_payhere) {
        this.makeToast('info', this.$t('Payment_processing_via_PayHere'), this.$t('Info'));
        // Don't show invoice yet - wait for notification
    } else {
        // Show invoice immediately for non-PayHere payments
        if (saleId) {
            this.Invoice_POS(saleId);
        }
    }
...
$color-gradient-primary: linear-gradient(135deg, #0963AC 0%, #09B3E9 100%);
$color-gradient-hover: linear-gradient(135deg, #084d8a 0%, #0797c7 100%);
...
.points-actions { display: flex; align-items: center; }
.convert-btn {
  display: inline-flex;
  align-items: center;
  gap: 6px;
...
@media (max-width: 576px) {
  ...
  .points-actions { justify-content: flex-start; }
  .points-value { justify-self: start; }
}
...
@media (max-width: 768px) {
  .pos-header { gap: 12px; padding: 12px 16px; min-height: auto; position: static !important; height: 126px; }
  .pos-footer-bar { position: static; }
...
.premium-payment-modal {
  --color-primary: #0963AC;
  --color-secondary: #09B3E9;
  --color-success: #10b981;
  --color-danger: #ef4444;
  --color-warning: #f59e0b;
  --color-border: #e5e7eb;
