...
export default {
  ...
  computed: {
    ...
    columns() {
      return [
        ...
        { label: this.$t("Vehicle_Number"), field: "vehicle_number", sortable: true, tdClass: "text-left", thClass: "text-left" },
        { label: this.$t("Meter_Reading"),  field: "meter_reading",  sortable: true, tdClass: "text-left", thClass: "text-left" },
  ...
  methods: {
    ...
    money(v){
      const n = Number(v||0);
      try { return new Intl.NumberFormat(undefined,{style:'currency',currency:this.currency}).format(n); }
      catch { return `${this.currency} ${n.toLocaleString()}`; }
    },
    ...
    async exportPDF(){
      try{
        ...
        const head = [[
          ...
          this.$t('warehouse'),
          this.$t('User'),
          this.$t('Vehicle_Number'),
          this.$t('Meter_Reading'),
          this.$t('Amount'),
          this.$t('Tax'),
          this.$t('Discount'),
          this.$t('Shipping'),
        ...
        autoTable(doc, {
          ...
          foot: [[
            { content: this.$t('Totals'), colSpan: 7, styles:{ font: 'Vazirmatn', fontStyle:'bold', halign: rtl ? 'right' : 'left' } },
