<template>
  <div class="main-content">
    ...
    <!-- Text.lk SMS API -->
    <validation-observer ref="textlk_form_sms" v-if="!isLoading">
    <b-form @submit.prevent="Submit_Textlk_SMS">
        <b-row class="mt-5">
        <b-col lg="12" md="12" sm="12">
            <b-card no-body header="Text.lk">
            <b-card-body>
                <b-row>
                ...
                <b-col lg="6" md="6" sm="12">
                    ...
                    v-slot="validationContext"
                    >
                    <b-form-group label="Text.lk API KEY *">
                        <b-form-input
                        :state="getValidationState(validationContext)"
                        aria-describedby="TEXTLK_API_KEY-feedback"
                        v-model="textlk.TEXTLK_API_KEY"
                        ></b-form-input>
                        <b-form-invalid-feedback id="TEXTLK_API_KEY-feedback">
                ...
                <b-col lg="6" md="6" sm="12">
                    <validation-provider
                    name="TEXTLK_SENDER_ID"
                    :rules="{ required: true}"
                    v-slot="validationContext"
                    >
                    <b-form-group label="Text.lk Sender ID *">
                        <b-form-input
                        :state="getValidationState(validationContext)"
                        aria-describedby="TEXTLK_SENDER_ID-feedback"
                        v-model="textlk.TEXTLK_SENDER_ID"
                        ></b-form-input>
                        <b-form-invalid-feedback id="TEXTLK_SENDER_ID-feedback">
                        {{ validationContext.errors[0] }}
                        </b-form-invalid-feedback>
                    </b-form-group>
                    </validation-provider>
...
export default {
  ...
  methods: {
    ...
    //------------- Submit Validation SMS
    Submit_Textlk_SMS() {
    this.$refs.textlk_form_sms.validate().then(success => {
        if (!success) {
        this.makeToast(
            "danger",
            this.$t("Please_fill_the_form_correctly"),
    ...
    //---------------------------------- update_textlk_config ----------------\\
    update_textlk_config() {
        NProgress.start();
        NProgress.set(0.1);
        axios
            .post("update_textlk_config",{
            TEXTLK_API_KEY: this.textlk.TEXTLK_API_KEY,
            TEXTLK_SENDER_ID: this.textlk.TEXTLK_SENDER_ID,
            })
            .then(response => {
            Fire.$emit("Event_sms");
            this.makeToast(
    ...
    get_sms_config() {
      axios
        ...
        .then(response => {
          ...
          this.textlk = response.data.textlk; 
