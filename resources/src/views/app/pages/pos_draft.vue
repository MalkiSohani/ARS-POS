    ...
    <div class="container-fluid p-0 app-admin-wrap layout-sidebar-large clearfix" id="pos">
      ...
      <b-row v-if="!isLoading">
                ...
                <b-card-body>
                  ...
                  <div class="footer_panel">
                    <b-row>
                      ...
                      <!-- Discount -->
                      <b-col lg="6" md="6" sm="12" v-if="currentUserPermissions && currentUserPermissions.includes('edit_tax_discount_shipping_sale')">
                        <validation-provider
                          name="Discount"
                          :rules="{ regex: /^\d*\.?\d*$/}"
                      ...
                      <!-- Points -->
                      <b-col lg="6" md="6" sm="12" v-if="clientIsEligible && currentUserPermissions && currentUserPermissions.includes('edit_tax_discount_shipping_sale')">
                      <b-form-group label="Available Points">
                        <b-input-group>
                          <b-form-input
                          ...
                          />
                          <b-input-group-append>
                            <b-button :style="pointsConverted ?'border: 1px solid #9CA3AF':''"
                              :variant="pointsConverted ? 'secondary' : 'primary'"
                              @click="convertPointsToDiscount"
                              :disabled="selectedClientPoints === 0"
                            >
                              {{ pointsConverted ? '✅' : $t('Convert') }}
                            </b-button>
                          </b-input-group-append>
                        </b-input-group>
                        <small v-if="discount_from_points > 0" class="text-success d-block mt-1">
                          ✅ {{ $t('Discount') }} {{ discount_from_points }} {{ currentUser.currency }} will be applied
                        </small>
        ...
        <b-col md="7">
          ...
          <div
              class="pos-button-actions"
              style="position: sticky; bottom: 0; z-index: 10; background: white; padding: 10px 15px; border-top: 1px solid #dee2e6; display: flex; flex-wrap: wrap; gap: 10px; justify-content: flex-start;"
            >              
            <b-button style="min-width: 120px;"
...
export default {
  ...
  convertPointsToDiscount() {
    ...
    formatNumber(number, dec) {
      const n = (number === null || number === undefined || number === '') ? 0 : number;
      const asString = (typeof n === "string" ? n : (typeof n.toString === 'function' ? n.toString() : String(n)));
      const parts = asString.split(".");
      if (!Array.isArray(parts) || parts.length === 0) return "0";
      if (dec <= 0) return parts[0];
      let fraction = parts[1] || "";
      if (fraction.length > dec) return `${parts[0]}.${fraction.substr(0, dec)}`;
    ...
    GetElementsPos() {
      ...
      axios
        ...
        .then(response => {
          ...
          this.selectedClientId = response.data.client_id;
