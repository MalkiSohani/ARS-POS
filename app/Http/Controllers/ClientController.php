...
class ClientController extends BaseController
{
    ...
    public function update(Request $request, $id)
    {
        ...
        $validated = $request->validate([
            ...
            'email'     => [
                ...
                // ensure ecommerce_clients.email unique per client (ignore current by client_id)
                Rule::unique('ecommerce_clients', 'email')->ignore($id, 'client_id'),
            ],
    ...
    public function import(Request $request)
    {
        ...
        $normalized = [];
        if ($assocInput) {
            foreach ($sheet as $r) $normalized[] = $this->normalizeAssocRow($r);
        } else {
            $header = array_map(function ($h) { return $this->normalizeKey((string) $h); }, $first);
            for ($i = 1; $i < count($sheet); $i++) {
                $row = $sheet[$i];
                $assoc = [];
                foreach ($header as $idx => $key) $assoc[$key] = $row[$idx] ?? null;
        ...
        foreach ($normalized as $i => $row) {
            ...
            } else if (is_numeric($codeRaw) && intval($codeRaw) == $codeRaw) {
                $code = intval($codeRaw);
            } else {
        ...
        // DB duplicates
        $codes = array_values(array_filter(array_map(function ($r) { return $r['code']; }, $prepared), function ($v) { return $v !== null; }));
        if (!empty($codes)) {
            $dupes = Client::whereNull('deleted_at')->whereIn('code', $codes)->pluck('code')->all();
            foreach ($dupes as $dup) $errors[] = "code '{$dup}' already exists in the system.";
        }
    ...
    {
        $request->validate([
            'id'     => 'required|integer|exists:sales,client_id', // at least one sale exists for this client; adjust if needed
            'limit'  => 'sometimes|integer',
            'page'   => 'sometimes|integer',
            'search' => 'sometimes|string|nullable',
        ]);
        ...
        $totalRows = (clone $payments)->count();
    ...
    public function clientBrief(Request $request, $id)
    {
        ...
        $client = Client::query()
            ->select('id', 'name', 'email', 'phone', 'code', 'adresse', 'country', 'city', 'tax_number')
            ->whereNull('deleted_at')
            ->findOrFail($id);
