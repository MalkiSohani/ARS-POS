...
class ProductsController extends BaseController
{
    ...
    public function index(Request $request)
    {
        ...
        foreach ($products as $product) {
            ...
            if ($product->type === 'is_single') {
                ...
                $item['cost']  = $variants->map(fn ($v) => number_format((float) $v->cost, 2, '.', ','))->implode("\n");
                $item['price'] = $variants->map(fn ($v) => number_format((float) $v->price, 2, '.', ','))->implode("\n");
                $item['unit']  = optional($product->unit)->ShortName;
    ...
    public function import_single_products(Request $request)
    {
        ...
        foreach ($rows as $row) {
            ...
            if (!is_numeric($price)) return response()->json(['status' => false, 'message' => "Retail price for \"$name\" is invalid."]);
    ...
    public function duplicate(Request $request, $id)
    {
        ...
        DB::transaction(function () use ($id, &$newProductId) {
            $original = Product::whereNull('deleted_at')
                ->with(['variants' => function ($q) { $q->whereNull('deleted_at'); }])
                ->findOrFail($id);
    ...
    protected function ensureUniqueCode(string $candidate): string
    {
        ...
        for ($i = 2; $i <= 10; $i++) {
            ...
            if (!$this->codeExistsAcrossTables($try)) return $try;
        }
