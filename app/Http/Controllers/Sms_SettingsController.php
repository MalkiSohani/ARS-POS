...
class Sms_SettingsController extends Controller
{
    ...
    public function get_sms_config(Request $request)
    {
        ...
        $textlk['TEXTLK_API_KEY'] = env('TEXTLK_API_KEY');
        $textlk['TEXTLK_SENDER_ID'] = env('TEXTLK_SENDER_ID');
    ...
    public function update_textlk_config(Request $request)
    {
        $this->authorizeForUser($request->user('api'), 'sms_settings', Setting::class);

        $this->setEnvironmentValue([
            'TEXTLK_API_KEY' => $request['TEXTLK_API_KEY'] !== null ? '"' . $request['TEXTLK_API_KEY'] . '"' : '"' . env('TEXTLK_API_KEY') . '"',
            'TEXTLK_SENDER_ID' => $request['TEXTLK_SENDER_ID'] !== null ? '"' . $request['TEXTLK_SENDER_ID'] . '"' : '"' . env('TEXTLK_SENDER_ID') . '"',
        ]);
