...
class SalesController extends BaseController
{
    ...
    public function destroy(Request $request, $id)
    {
        ...
        \DB::transaction(function () use ($id, $request, &$qboInfo) {
            $role         = \Auth::user()->roles()->first();
            $view_records = \App\Models\Role::findOrFail($role->id)->inRole('record_view');
            ...
            if ($current->statut === 'completed') {
                foreach ($current->details as $d) {
                    $product = $d->product;
                    if (!$product || ($product->type ?? null) === 'is_service') continue;
    ...
    public function Print_Invoice_POS(Request $request, $id)
    {
        ...
        $item['GrandTotal'] = number_format($sale->GrandTotal, 2, '.', '');
        $item['paid_amount'] = number_format($sale->paid_amount, 2, '.', '');
        $item['vehicle_number'] = $sale->vehicle_number;
        $item['meter_reading'] = $sale->meter_reading;
        $item['job_no'] = $sale->job_no;
        $item['time_in'] = $sale->time_in;
        $item['time_out'] = $sale->time_out;
        $item['next_service_due'] = $sale->next_service_due;
