...
class PosController extends BaseController
{
    ...
    public function CreatePOS(Request $request)
    {
        ...
        $sale = \DB::transaction(function () use ($request, $totalPaid) {
            $helpers = new helpers();
            ...
            $selectedCardsByLine = $request->selected_cards_by_line ?? [];
    ...
    if (isset($payment['amount']) && $payment['amount'] > 0) {
        ...
        // ✅ Single payment creation (no duplicate)
        $paymentSale = PaymentSale::create([
            ...
            'status' => ($payment['payment_method_id'] == 1) ? 'pending' : 'paid', // ✅ Mark PayHere as pending
        ]);

        // ✅ Only update account balance for non-PayHere payments
        if ($payment['payment_method_id'] != 1 && $accountId) {
        ...
        // Check if PayHere payment exists

        $hasPayherePayment = false;
        $payhereAmount = 0;

        foreach ($request->payments ?? [] as $payment) {
            if (isset($payment['payment_method_id']) && $payment['payment_method_id'] == 1) {
                $hasPayherePayment = true;
                $payhereAmount += floatval($payment['amount'] ?? 0);
            }
        }

        if ($hasPayherePayment && $payhereAmount > 0) {
            $merchant_id = env('PAYHERE_MERCHANT_ID');
            $merchant_secret = env('PAYHERE_MERCHANT_SECRET');
            $mode = env('PAYHERE_MODE', 'sandbox');

            // ✅ Use sale ID without prefix
            $order_id = (string)$sale->id;

            // ✅ Format amount to exactly 2 decimal places
            $amount = number_format((float)$payhereAmount, 2, '.', '');
            $currency = 'LKR';

            // ✅ Generate hash
            $merchant_secret_hash = strtoupper(md5($merchant_secret));
            // ✅ CORRECT - Amount properly formatted
            $hash_string = $merchant_id . $order_id . number_format((float)$amount, 2, '.', '') . $currency . $merchant_secret_hash;
            $hash = strtoupper(md5($hash_string));

            $client = Client::find($request->client_id);

            $payhere_data = [
                'action_url' => $mode === 'live'
                ...
                'order_id' => $order_id,  // ✅ Will be just the number (e.g., "1167")
                'items' => 'POS Sale #' . $sale->Ref,
                'currency' => $currency,
                'amount' => $amount,
                ...
                'address' => $client->adresse ?? 'No Address',
                'city' => $client->city ?? 'Colombo',
                'country' => 'Sri Lanka',
                'hash' => $hash,
                'custom_1' => (string)$sale->id,
                'custom_2' => ''
            ];

            // ✅ Detailed logging
            \Log::info('PayHere Payment Request', [
                'sale_id' => $sale->id,
                'order_id' => $order_id,
                'amount' => $amount,
                'currency' => $currency,
